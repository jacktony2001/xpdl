...
class VideoScraper:
    ...
    def _scroll_to_load_more(self):
        ...
        if scroll_times > 0:
            ...
            for _ in range(scroll_times):
                ...
                # زمان انتظار برای اسکرول کردن، از config.py گرفته میشود
                time.sleep(SCRAPER_CONFIG.get("wait_time_scroll", 3)) 
    ...
    def get_video_page_links_from_homepage(self):
        ...
        try:
            ...
            # زمان انتظار برای لود شدن صفحه اصلی
            time.sleep(SCRAPER_CONFIG.get("wait_time_homepage", 7)) 
    ...
    def get_final_video_url(self, video_page_url):
        ...
        try:
            ...
            # منتظر میمونیم تا صفحه اصلی ویدیو لود بشه (مقدار از config.py گرفته میشود)
            time.sleep(SCRAPER_CONFIG.get("wait_time_video_page", 10)) 
            ...
            # --- اضافه کردن انتظار اضافی برای لود کامل محتوای دانلود ---
            extra_wait_for_download_content = SCRAPER_CONFIG.get("extra_wait_for_download_content", 5)
            time.sleep(extra_wait_for_download_content) 
            logger.info(f"✅ منتظر لود کامل محتوای دانلود (اضافی: {extra_wait_for_download_content} ثانیه) ماندیم...")
            # ---------------------------------------------------------------
            ...
            if download_elements:
                # معمولاً لینک اولMEDIUM است، اگر نیاز به LOW بود باید منطق تغییر کند
    ...
    def scrape(self):
        ...
        # تعداد ویدیوهایی که از صفحه اصلی پردازش میشوند (مثلا 10 تا)
        max_videos_to_process = SCRAPER_CONFIG.get("max_videos_per_run", 10) 
